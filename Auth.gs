...
function getDashboardSummary(payload) {
  Logger.log('getDashboardSummary CALLED with payload: ' + JSON.stringify(payload));
  try {
    validateRequired_(payload, ['nik']);
    const nik = normalizeNik_(payload.nik);
    const user = getUserProfile_(nik);
    ...
    result.streak = getUserStreak_(nik);
    Logger.log('getDashboardSummary RETURNING OK for ' + nik + ': ' + JSON.stringify(result));
    return result;
  } catch (e) {
    Logger.log('getDashboardSummary error: ' + (e && e.stack || e));
